...
import { Download, Upload, CheckCircle, Clock, User, MapPin, Calendar, IndianRupee, Phone, Mail, X, AlertCircle } from 'lucide-react';
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    ...
    const orderIdParam = urlParams.get('orderId') || 'ORD' + Date.now();
    const amountParam = parseFloat(urlParams.get('amount')) || 117;
    
    setOrderId(orderIdParam);
    setAmount(amountParam);

    fetchPaymentDetails(orderIdParam);
  }, []);
  ...
  const fetchPaymentDetails = async (orderId) => {
    try {
      ...
      const response = await fetch(`/api/payment-details?orderId=${orderId}`);
  ...
  const downloadQR = () => {
    ...
    link.href = qrCode;
    link.download = `VR-Darshan-Payment-QR-${orderId}.png`;
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      // Send payment data to backend (fire and forget)
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
              ...
              <button
                ...
                className={`w-full mt-6 px-6 py-4 rounded-lg font-bold text-lg transition-all transform ${
